  ...
  <div>
    <Header title="Verifikasi" :left-action="true">
      <template #left-action>
        <NuxtLink to="/account/change/email/new">
          <svg
            width="37"
            height="40"
            viewBox="0 0 37 40"
          ...
          >
            <div class="text-center">
              <p class="text-xs text-grey-2 max-w-[269px] mx-auto font-medium">
                Kami telah mengirimkan kode verifikasi melalui alamat email
                berikut :
              </p>
            ...
            <div>
              <div class="px-3 pb-8 text-center pt-7">
                ...
                <p class="mt-2 text-xs text-grey-2 max-w-[222px] mx-auto">
                  Alamat email Anda berhasil diperbarui ke email baru
                </p>
              </div>
              <div
...
export default {
  ...
  computed: {
    emailNew() {
      return this.$store.state.account.change.email.new.value
    },
    nextTimeRequestCode() {
  ...
  mounted() {
    if (!this.$store.state.account.change.email.new.value) {
      this.$router.push('/account/change/email/new')
      return
    }
